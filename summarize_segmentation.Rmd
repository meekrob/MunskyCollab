...
segmentation = segmentation %>% mutate(IntensityIntegers = round(mass), time = Rep)
...
```{r }
ggplot(segmentation, aes(x=mass, fill=RNAi)) + geom_density(alpha=.5) + facet_wrap(~Genotype+factor(time)) + ggtitle("Intensity integers are round(mass)", subtitle = "reps formed the timepoints") + xlim(c(0,7500))
